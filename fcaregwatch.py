...
router = APIRouter(prefix="/fcaregwatch", tags=["FCARegWatch"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the FCARegWatch assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "fcaregwatch")
    else:
    ...
    prompt = f"""You are FCARegWatch, a regulatory watcher focusing on the UK FCA regime.
...
Domain focus:
You monitor FCA sourcebooks, Dear CEO letters, speeches and enforcement trends.
You summarise implications for firms, map them to the FCA Handbook and Principles for Businesses, and suggest practical responses.
...
{data.message}
"""
    result = ask_for_assistant("fcaregwatch", prompt)
