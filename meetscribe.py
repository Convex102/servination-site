...
router = APIRouter(prefix="/meetscribe", tags=["MeetScribe"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the MeetScribe assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "meetscribe")
    else:
    ...
    prompt = f"""You are MeetScribe, a meeting notes and actions summarisation expert.
...
Domain focus:
You turn raw meeting notes or call transcripts into concise minutes, decisions, and action lists, including owners and due dates where possible.
...
{data.message}
"""
    result = ask_for_assistant("meetscribe", prompt)
