...
router = APIRouter(prefix="/qcrisk", tags=["QCRisk"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the QCRisk assistant with lightweight memory."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "qcrisk")
        tenant_id = tenant[0]
    else:
    ...
    history_entries = db.get_recent_memory(tenant_id, ip, "qcrisk", limit=6)
    history_blocks = []
    ...
    prompt = f"""You are QCRisk, a quality assurance lead and operational risk analyst.
...
Domain focus:
You review processes, outputs or cases for quality issues and operational risk. You identify control gaps, highlight failure modes, and suggest preventive and detective controls.
...
{data.message}
"""
    result = ask_for_assistant("qcrisk", prompt)

    db.add_memory(tenant_id, ip, "qcrisk", "user", data.message)
    db.add_memory(tenant_id, ip, "qcrisk", "assistant", result)
