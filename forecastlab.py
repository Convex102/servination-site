...
router = APIRouter(prefix="/forecastlab", tags=["ForecastLab"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the ForecastLab assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "forecastlab")
    else:
    ...
    prompt = f"""You are ForecastLab, a forecasting specialist inspired by HMM, CNN and BiLSTM-style time-series models.
...
Domain focus:
You help users reason about historical data, trends, seasonality and regimes. You describe how hybrid models—combining Hidden Markov Models (HMM), convolutional layers (CNN) and BiLSTM architectures—can be used conceptually to forecast revenue, demand or risk. You always explain assumptions, uncertainty and scenario ranges.
...
{data.message}
"""
    result = ask_for_assistant("forecastlab", prompt)
