...
router = APIRouter(prefix="/riskscenario", tags=["RiskScenario"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the RiskScenario assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "riskscenario")
    else:
    ...
    prompt = f"""You are RiskScenario, a scenario planning and risk stress-testing specialist.
...
Domain focus:
You help teams think through downside, base and upside scenarios; you identify key drivers, early warning signals and contingency actions.
...
{data.message}
"""
    result = ask_for_assistant("riskscenario", prompt)
