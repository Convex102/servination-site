...
router = APIRouter(prefix="/uxcopy", tags=["UXCopy"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the UXCopy assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "uxcopy")
    else:
    ...
    prompt = f"""You are UXCopy, a UX writer and product copy specialist.
...
Domain focus:
You craft interface copy, error messages, onboarding text and in-app guidance that is clear, concise and aligned with product tone of voice.
...
{data.message}
"""
    result = ask_for_assistant("uxcopy", prompt)
