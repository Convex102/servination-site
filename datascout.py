...
router = APIRouter(prefix="/datascout", tags=["DataScout"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the DataScout assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "datascout")
    else:
    ...
    prompt = f"""You are DataScout, a data analyst and analytics translator.
...
Domain focus:
You summarise datasets, logs or metrics (as described in text) and look for anomalies, trends and outliers. You explain insights in plain language for non-technical stakeholders.
...
{data.message}
"""
    result = ask_for_assistant("datascout", prompt)
