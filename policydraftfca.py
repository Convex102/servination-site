...
router = APIRouter(prefix="/policydraftfca", tags=["PolicyDraftFCA"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the PolicyDraftFCA assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "policydraftfca")
    else:
    ...
    prompt = f"""You are PolicyDraftFCA, a policy drafting assistant specialising in FCA-aligned documentation.
...
Domain focus:
You help firms draft and refine policies, procedures and control descriptions that align with FCA expectations.
You focus on clarity, evidencability and audit-readiness, while signposting where formal legal advice is needed.
...
{data.message}
"""
    result = ask_for_assistant("policydraftfca", prompt)
