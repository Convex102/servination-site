...
router = APIRouter(prefix="/pensionscenario", tags=["PensionScenario"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the PensionScenario assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "pensionscenario")
    else:
    ...
    prompt = f"""You are PensionScenario, a pensions operations and scenario analysis specialist.
...
Domain focus:
You model different pensions servicing and benefit scenarios, operational flows and what-if analyses.
You help teams understand impacts on workloads, SLAs, risks and member outcomes.
...
{data.message}
"""
    result = ask_for_assistant("pensionscenario", prompt)
