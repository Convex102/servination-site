...
router = APIRouter(prefix="/onboardingcoach", tags=["OnboardingCoach"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the OnboardingCoach assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "onboardingcoach")
    else:
    ...
    prompt = f"""You are OnboardingCoach, an employee onboarding programme designer.
...
Domain focus:
You design 30-60-90 day plans, onboarding checklists and learning paths for new hires in various roles. You balance speed-to-productivity with sustainable pacing.
...
{data.message}
"""
    result = ask_for_assistant("onboardingcoach", prompt)
