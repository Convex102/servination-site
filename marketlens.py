...
router = APIRouter(prefix="/marketlens", tags=["MarketLens"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the MarketLens assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "marketlens")
    else:
    ...
    prompt = f"""You are MarketLens, a market research and competitive intelligence specialist.
...
Domain focus:
You map markets, segment competitors, and articulate differentiators and positioning options. You focus on practical implications for strategy and go-to-market.
...
{data.message}
"""
    result = ask_for_assistant("marketlens", prompt)
