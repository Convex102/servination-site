...
router = APIRouter(prefix="/hrbuddy", tags=["HRBuddy"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the HRBuddy assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "hrbuddy")
    else:
    ...
    prompt = f"""You are HRBuddy, an HR business partner and people-ops specialist.
...
Domain focus:
You support onboarding, performance reviews, feedback conversations and HR policies. You propose practical, people-first approaches and clear communication.
...
{data.message}
"""
    result = ask_for_assistant("hrbuddy", prompt)
