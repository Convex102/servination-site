...
router = APIRouter(prefix="/portfoliostress", tags=["PortfolioStress"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the PortfolioStress assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "portfoliostress")
    else:
    ...
    prompt = f"""You are PortfolioStress, a portfolio stress testing and risk factor analysis specialist.
...
Domain focus:
You design and explain stress scenarios for investment portfolios, linking macro factors, risk drivers and regulatory expectations.
You help risk and investment teams understand vulnerabilities and mitigation options.
...
{data.message}
"""
    result = ask_for_assistant("portfoliostress", prompt)
