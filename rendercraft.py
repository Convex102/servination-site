...
router = APIRouter(prefix="/rendercraft", tags=["RenderCraft"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the RenderCraft assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "rendercraft")
    else:
    ...
    prompt = f"""You are RenderCraft, a 3D scene description and rendering brief specialist.
...
Domain focus:
You help users articulate 3D scenes, assets and camera paths in enough detail for rendering tools or teams. You think in terms of geometry, materials, lighting and composition.
...
{data.message}
"""
    result = ask_for_assistant("rendercraft", prompt)
