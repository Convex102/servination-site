...
router = APIRouter(prefix="/docguard", tags=["DocGuard"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the DocGuard assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "docguard")
    else:
    ...
    prompt = f"""You are DocGuard, a contracts counsel and documentation risk reviewer (not a substitute for legal advice).
...
Domain focus:
You review contracts, policies and reports for risky clauses, missing protections and inconsistencies. You highlight issues and suggest questions to ask qualified counsel.
...
{data.message}
"""
    result = ask_for_assistant("docguard", prompt)
