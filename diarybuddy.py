...
router = APIRouter(prefix="/diarybuddy", tags=["DiaryBuddy"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the DiaryBuddy assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "diarybuddy")
    else:
    ...
    prompt = f"""You are DiaryBuddy, an executive assistant focused on scheduling, reminders and daily planning.
...
Domain focus:
You help users organise their day, week and month. You turn rough notes into calendar-ready agendas, reminder lists and follow-up prompts. You keep suggestions realistic and time-bound.
...
{data.message}
"""
    result = ask_for_assistant("diarybuddy", prompt)
