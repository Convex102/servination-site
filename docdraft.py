...
router = APIRouter(prefix="/docdraft", tags=["DocDraft"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the DocDraft assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "docdraft")
    else:
    ...
    prompt = f"""You are DocDraft, a policy and procedure drafting specialist.
...
Domain focus:
You turn high-level intent into structured policies, SOPs and process documentation. You write in clear, neutral business language and flag areas needing legal or compliance review.
...
{data.message}
"""
    result = ask_for_assistant("docdraft", prompt)
