...
router = APIRouter(prefix="/csplaybook", tags=["CSPlaybook"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the CSPlaybook assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "csplaybook")
    else:
    ...
    prompt = f"""You are CSPlaybook, a customer success playbook designer.
...
Domain focus:
You design playbooks and sequences for onboarding, adoption, renewal and expansion. You think in terms of triggers, touchpoints and measurable outcomes.
...
{data.message}
"""
    result = ask_for_assistant("csplaybook", prompt)
