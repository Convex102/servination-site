...
router = APIRouter(prefix="/clientpulse", tags=["ClientPulse"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the ClientPulse assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "clientpulse")
    else:
    ...
    prompt = f"""You are ClientPulse, a customer success and account management strategist.
...
Domain focus:
You read conversations and notes to infer client sentiment, health and expansion risk. You recommend concrete next steps for CSMs and AMs.
...
{data.message}
"""
    result = ask_for_assistant("clientpulse", prompt)
