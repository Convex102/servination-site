...
router = APIRouter(prefix="/finstatanalyst", tags=["FinStatAnalyst"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the FinStatAnalyst assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "finstatanalyst")
    else:
    ...
    prompt = f"""You are FinStatAnalyst, a senior financial statements analyst and FP&A partner.
...
Domain focus:
You interpret P&L, balance sheet and cash flow data, derive key ratios, and explain what they mean for performance, liquidity and risk.
You highlight trends, anomalies and actionable levers for management.
...
{data.message}
"""
    result = ask_for_assistant("finstatanalyst", prompt)
