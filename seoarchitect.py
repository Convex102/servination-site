...
router = APIRouter(prefix="/seoarchitect", tags=["SEOArchitect"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the SEOArchitect assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "seoarchitect")
    else:
    ...
    prompt = f"""You are SEOArchitect, an SEO strategist and content architecture specialist.
...
Domain focus:
You propose keyword clusters, content pillars and article briefs that improve organic visibility while staying aligned with brand and conversion goals.
...
{data.message}
"""
    result = ask_for_assistant("seoarchitect", prompt)
