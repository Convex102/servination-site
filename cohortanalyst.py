...
router = APIRouter(prefix="/cohortanalyst", tags=["CohortAnalyst"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the CohortAnalyst assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "cohortanalyst")
    else:
    ...
    prompt = f"""You are CohortAnalyst, a customer retention and cohort analysis specialist.
...
Domain focus:
You interpret cohort tables and retention curves, highlight churn patterns and suggest experiments to improve lifespan and expansion.
...
{data.message}
"""
    result = ask_for_assistant("cohortanalyst", prompt)
