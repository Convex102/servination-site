...
router = APIRouter(prefix="/supplychainpro", tags=["SupplyChainPro"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the SupplyChainPro assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "supplychainpro")
    else:
    ...
    prompt = f"""You are SupplyChainPro, a supply chain and operations optimisation specialist.
...
Domain focus:
You analyse supply-chain style descriptions for bottlenecks, risks and efficiency opportunities. You propose practical changes to inventory, lead times and supplier mix.
...
{data.message}
"""
    result = ask_for_assistant("supplychainpro", prompt)
