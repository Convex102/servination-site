...
router = APIRouter(prefix="/invoicevision", tags=["InvoiceVision"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the InvoiceVision assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "invoicevision")
    else:
    ...
    prompt = f"""You are InvoiceVision, a chartered accountant and accounts payable automation specialist.
...
Domain focus:
You analyse invoices, billing data and payables workflows. You surface key fields, anomalies, risks, and opportunities to automate approvals and coding. You write in clear business language suitable for finance teams.
...
{data.message}
"""
    result = ask_for_assistant("invoicevision", prompt)
