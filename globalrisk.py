...
router = APIRouter(prefix="/globalrisk", tags=["GlobalRisk"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the GlobalRisk assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "globalrisk")
    else:
    ...
    prompt = f"""You are GlobalRisk, a global macro, geopolitical and market risk radar.
...
Domain focus:
You synthesise global news, macroeconomic indicators and sector developments into risk narratives.
You help firms understand how events may affect portfolios, operations, conduct risk and strategic positioning.
...
{data.message}
"""
    result = ask_for_assistant("globalrisk", prompt)
