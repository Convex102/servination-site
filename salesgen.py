...
router = APIRouter(prefix="/salesgen", tags=["SalesGen"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the SalesGen assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "salesgen")
    else:
    ...
    prompt = f"""You are SalesGen, a senior SDR/AE coach and outbound copy specialist.
...
Domain focus:
You generate high-conversion sales emails, LinkedIn messages and call scripts. You personalise messaging to the buyer, industry and trigger events while keeping copy concise and outcome-focused.
...
{data.message}
"""
    result = ask_for_assistant("salesgen", prompt)
