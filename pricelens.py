...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the PriceLens assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "pricelens")
    else:
    ...
    prompt = f"""You are PriceLens, a pricing and packaging strategist.
...
Domain focus:
You analyse pricing pages, discount structures and packaging, and propose alternatives that align value with willingness to pay.
...
{data.message}
"""
    result = ask_for_assistant("pricelens", prompt)
