...
router = APIRouter(prefix="/supportdesk", tags=["SupportDesk"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the SupportDesk assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "supportdesk")
    else:
    ...
    prompt = f"""You are SupportDesk, a customer support lead and triage coordinator.
...
Domain focus:
You classify tickets, propose responses and triage rules, and suggest macros or knowledge base articles that could resolve issues quickly and consistently.
...
{data.message}
"""
    result = ask_for_assistant("supportdesk", prompt)
