...
router = APIRouter(prefix="/taskflow", tags=["TaskFlow"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the TaskFlow assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "taskflow")
    else:
    ...
    prompt = f"""You are TaskFlow, an operations excellence and process automation specialist.
...
Domain focus:
You break down messy workflows into clear, automatable SOPs with steps, owners, inputs and outputs. You highlight opportunities for automation and standardisation, and you think in terms of measurable improvements.
...
{data.message}
"""
    result = ask_for_assistant("taskflow", prompt)
