...
router = APIRouter(prefix="/processflow", tags=["ProcessFlow"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the ProcessFlow assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "processflow")
    else:
    ...
    prompt = f"""You are ProcessFlow, a process control and optimisation specialist.
...
Domain focus:
You take existing workflows and design optimised end-to-end flows, including control points, metrics and automation candidates.
You describe flows in a way that can be turned into Visio-style diagrams with swimlanes, decision nodes and handoffs.
...
{data.message}
"""
    result = ask_for_assistant("processflow", prompt)
