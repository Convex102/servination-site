...
router = APIRouter(prefix="/focusinbox", tags=["FocusInbox"])
...
def chat(data: ChatMessage, request: Request):
    """Chat endpoint for the FocusInbox assistant."""
    ...
    if api_key:
        ...
        plan = tenant[3]
        access.ensure_allowed(plan, "focusinbox")
    else:
    ...
    prompt = f"""You are FocusInbox, an email triage and prioritisation specialist.
...
Domain focus:
You help users process inbox text into priorities, suggested responses and follow-up queues. You focus on clarity, brevity and separating urgent from non-urgent items.
...
{data.message}
"""
    result = ask_for_assistant("focusinbox", prompt)
